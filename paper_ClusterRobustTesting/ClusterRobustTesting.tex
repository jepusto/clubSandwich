...
In the latter case, the errors are assumed to follow some known structure, $\Var\left(\bm{e}_j\left|\bm{X}_j\right.\right) = \bs\Phi_j$, where $\bs\Phi_j$ is a known function of a low-dimensional parameter. 
...
Assuming that $\bm{X}_j$ has rank $p$, their proposed approach involves estimating $\bs\beta$ separately within each cluster and taking the simple average of these estimates. 
...
One approach to estimating this variance would be to posit a working model--potentially the same working model used to construct weights--and substitute estimates of the working variance structure in place of $\bs\Sigma$. 
...
\end{equation}
If $\bs\beta$ is estimated using inverse-variance weights defined under the same working model, then the model-based variance estimator simplifies to $\bm{V}^M = \bm{M}$. 

Cluster-robust variance estimators provide a means of estimating $\Var\left(\bs{\hat\beta}\right)$ and test hypotheses regarding $\hat{\bs\beta}$ in the absence of a valid working model for the error structure, or if the working variance model used to develop weights is mis-specified. 
...
For example, the Stata command \texttt{regress} scales CR0 by the factor $m (N - 1) / [(m - 1) (N - p)]$, where $N = \sum_{j=1}^m n_j$. 

\subsection{Correction based on a working-model}

\citet{McCaffrey2001generalizations} proposed to correct the small-sample bias of CR0 so that it is exactly unbiased under a specified working model for the variance structure. 
In their implementation, the residuals from each cluster are multiplied by an $n_j \times n_j$ adjustment matrix $\bm{A}_j$ that creates the unbiasedness property. 
...
\end{equation}
where $\bs\Phi = \bigoplus_{j=1}^m \bs\Phi_j$, $\bm{H} = \bm{X}\bm{M}\bm{X}'\bm{W}$, and $\left(\bm{I} - \bm{H}\right)_j$ denotes the rows of $\bm{I} - \bm{H}$ corresponding to cluster $j$. 
...
and $\bm{B}_j^{-1/2}$ is the inverse of the symmetric square root of $\bm{B}$. 
If ordinary (unweighted) least squares is used to estimate $\bs{\hat\beta}$ and the working variance model posits that the errors are all independent and homoskedastic, then $\bm{W} = \bs\Phi = \bm{I}_N$ and $\bm{A}_j = \left(\bm{I}_j - \bm{H}_{jj}\right)^{-1/2}$, where $\bm{I}_j$ is an $n_j \times n_j$ identity matrix and $\bm{H}_{jj} = \bm{X}_j\left(\bm{X}'\bm{X}\right)^{-1}\bm{X}_j'$.
...
However, this problem is easily overcome by using a generalized inverse of $\bm{B}_j$. 
...
\end{equation}
where $\bm{F}_j = \bm{X}_j' \bm{W}_j \bs\Phi_j \bm{W}_j \bm{X}_j$ and $\bm{G}_j = \bm{X}_j' \bm{W}_j \left(\bm{I} - \bm{H}\right)_j \bs\Phi \left(\bm{I} - \bm{H}\right)_j' \bm{W}_j \bm{X}_j$. Note that $\bm{F}_j$ and $\bm{G}_j$ might not be positive definite, and so a generalized form of the Cholesky decomposition and symmetric inverse-square root must be used. The $\bm{D}_j$ correction matrices will be defined (I think) so long as $\text{rank}(\bm{F}_j) \geq \text{rank}(\bm{G}_j)$ for $j = 1,...,m$. In datasets where clusters have a large number of individual units, CR2b will be less computationally intensive than CR2a because the adjustment matrices are all of dimension $p \times p$.
