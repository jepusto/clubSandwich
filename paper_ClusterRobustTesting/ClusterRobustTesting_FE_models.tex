...
Although the error is above the simulation bound under some conditions, the departures are typically small. For example, when $m = 15$ the rejection rates do not exceed # A tibble: 1 x 1,   reject,    <dbl>, 1  0.012 for $\alpha = .01$, # A tibble: 1 x 1,   reject,    <dbl>, 1  0.055 for $\alpha = .05$, and # A tibble: 1 x 1,   reject,    <dbl>, 1  0.106 for $\alpha = .10$.
...
For example, the maximum Type I error ranges from # A tibble: 1 x 1,    `15`,   <dbl>, 1 0.127 ($q = 1$) to # A tibble: 1 x 1,    `15`,   <dbl>, 1 0.687 ($q = 6$) for data sets with 15 clusters.
Perhaps even more important for practice, the rejection rate of the standard test can be far above the stated $\alpha$ level even when there are 50 clusters, with maximum error ranging from # A tibble: 1 x 1,    `50`,   <dbl>, 1 0.071 ($q = 1$) to # A tibble: 1 x 1,    `50`,   <dbl>, 1 0.186 ($q = 6$). 
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in Wald\_test(ATE\_mod, constraints = ATE\_constraints, vcov = "{}CR1"{}, : 'list' object cannot be coerced to type 'integer'}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in xtable(AL\_results, caption = "{}Tests of treatment effects in the Achievement Awards Demonstration"{}, : object 'AL\_results' not found}}\end{kframe}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in if (!inherits(constraints, "{}matrix"{}) | ncol(constraints) != p) \{: argument is of length zero}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in xtable(MLDA\_results, caption = "{}Tests of effects of minimum legal drink age and Hausman specification test"{}, : object 'MLDA\_results' not found}}\end{kframe}
