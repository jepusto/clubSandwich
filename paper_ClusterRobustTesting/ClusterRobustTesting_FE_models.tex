...
the supplementary materials provide the exact specifications used. Table \ref{tab:simulation_parameters} summarizes the design of the simulation study, which included a total of 972 unique conditions.
...
Simulated Type I error rates therefore have standard errors of approximately 0.0004 for $\alpha = .01$, 0.0010 for $\alpha = .05$, and 0.0013 for $\alpha = .10$. 
...
Although the error is above the simulation bound under some conditions, the departures are typically small. For example, when $m = 15$ the rejection rates do not exceed 0.011 for $\alpha = .01$, 0.053 for $\alpha = .05$, and 0.106 for $\alpha = .10$.
...
For example, the maximum Type I error ranges from 0.122 ($q = 1$) to 0.717 ($q = 6$) for data sets with 15 clusters.
Perhaps even more important for practice, the rejection rate of the standard test can be far above the stated $\alpha$ level even when there are 50 clusters, with maximum error ranging from 0.070 ($q = 1$) to 0.188 ($q = 6$). 
...
In the top row, rejection rates of the AHT test are very close to nominal (between 0.03 and 0.056) across all conditions with at least 30 clusters.
