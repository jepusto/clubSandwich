...
Each combination of sample sizes and parameter levels was simulated for balanced and unbalanced versions of each of the three study designs, yielding a total of 648 unique conditions.
...
For a given combination of sample sizes, parameter levels, and study design, we simulated 20,000 datasets from model (\ref{eq:data_generating_model}), estimated model (\ref{eq:sim_analytic_model}) on each dataset, and computed all of the hypothesis tests. 
...
\caption[Rejection rates of ad hoc and AHT tests for ]{Rejection rates of ad hoc and AHT tests for $\alpha = .05$, by dimension of hypothesis ($q$) and sample size ($m$). The solid horizontal line indicates the stated $\alpha$ level and the dashed line indicates an upper confidence bound on simulation error.}\label{fig:overview}
\end{figure}
...
Although there exist situations in which the error is above the simulation bound, the departures are typically small. For example, when $m = 15$ the rejection rates do not exceed 0.013 for $\alpha = .01$, 0.055 for $\alpha = .05$, and 0.109 for $\alpha = .10$.
...
Perhaps even more important for practice, the rejection rate of the ad hoc test can be far above the stated $\alpha$ level even when there are 50 clusters, with maximum error ranging from 0.072 ($q = 1$) to 0.181 ($q = 6$). 
...
For studies with at least 30 clusters, rejection rates of the AHT test with CR2 are very close to nominal (between 0.029 and 0.056) across all conditions studied.
...
\begin{tabular}{lcrrr}
  \toprule
Hypothesis & Test & Fstat & df & p \\ 
  \midrule
ATE - upper half (q = 1) & ad hoc & 5.746 & 34.00 & 0.02217 \\ 
   & AHT & 5.169 & 18.13 & 0.03539 \\ 
   & AHT* & 4.825 & 19.38 & 0.04040 \\ 
  ATE - joint (q = 2) & ad hoc & 3.848 & 34.00 & 0.03116 \\ 
   & AHT & 3.389 & 16.97 & 0.05775 \\ 
   & AHT* & 3.307 & 17.67 & 0.06026 \\ 
   \midrule
Moderation - upper half (q = 2) & ad hoc & 3.186 & 34.00 & 0.05393 \\ 
   & AHT & 1.665 & 7.84 & 0.24959 \\ 
   & AHT* & 2.058 & 2.27 & 0.30916 \\ 
  Moderation - joint (q = 4) & ad hoc & 8.213 & 34.00 & 0.00010 \\ 
