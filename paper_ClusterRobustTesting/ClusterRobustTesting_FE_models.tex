...
\usepackage{booktabs}
\RequirePackage[natbibapa, mask]{apacite}
\usepackage{caption}
\usepackage{multirow}
...
In the state-by-year panel analysis described in the introduction, $\bm{R}_i$ would include indicator variables for each policy change, as well as additional demographic controls; $\bm{S}_i$ would include year fixed effects; and $\bm{T}_i$ would indicate state fixed effects. 
...
Clearly, the WLS estimator includes OLS as a special case, as well as feasible GLS.\footnote{
...
These estimators are now commonly used in empirical applications.
...
\citet{Angrist2009mostly} note that this problem occurs in balanced state-by-year panel models that include fixed effects for states and for years, in which case $\bm{I}_i - \bm{\ddot{R}}_i\bm{M_{\ddot{R}}} \bm{\ddot{R}}_i'$ is not of full rank. 
...
A symmetric solution to Equation (\ref{eq:CR2_criterion}) uses
\begin{equation}
...
\citet{Cameron2015practitioners} highlight that this discrepancy can be substantial if the clusters are small. 
%For instance, if each cluster includes $n_i = 2$ units, then the CR1S estimator based on the LSDV estimator is over twice as large as the estimator based on the absorbed model.
...
A drawback of using the CR2 adjustment matrices based on the full regression model is that it entails calculating the projection matrix $\bm{H_X}$ for the full set of $p$ covariates (i.e., including fixed effect indicators). 
...
Like the bias of the CRVE estimator itself, the accuracy of the asymptotic approximations depends on design features such as the degree of imbalance across clusters, skewness or leverage of the covariates, and the similarity of cluster sizes \citep[][\maskcitealp{Tipton2015small-F}]{McCaffrey2001generalizations, Webb2013wild, Carter2013asymptotic}. 
...
\end{equation}
where $\bm{p}_i = \left(\bm{I} - \bm{H_X}\right)_i'\bm{A}_i \bm{W}_i\bm{\ddot{R}}_i\bm{M_{\ddot{R}}} \bm{c}$.\footnote{Alternately, for any of the CRVEs one could instead use an ``empirical'' estimate of the degrees of freedom, constructed by substituting $\bm{e}_i \bm{e}_i'$ in place of $\bs\Phi_i$. 
However, \citet{Bell2002bias} found using simulation that this plug-in degrees of freedom estimate led to very conservative rejection rates.}\todo{Can we use $\bm{H_{\ddot{U}}}$ here instead?} 
