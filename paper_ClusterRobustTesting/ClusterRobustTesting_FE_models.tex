...
For a given combination of sample sizes, parameter levels, and study design, we simulated 10,000 datasets from model (\ref{eq:data_generating_model}), estimated model (\ref{eq:sim_analytic_model}) on each dataset, and tested all of the hypotheses described above. Simulated Type I error rates therefore have standard errors of approximately 0.001 for $\alpha = .01$, 0.002 for $\alpha = .05$, and 0.003 for $\alpha = .10$. 
...
\caption[Rejection rates of Naive and AHT tests, by dimension of hypothesis (]{Rejection rates of Naive and AHT tests, by dimension of hypothesis ($q$) and nominal type I error ($\alpha$)}\label{fig:overview}
\end{sidewaysfigure}
...
\caption[Rejection rates of Naive and AHT tests, by study design and dimension of hypothesis (]{Rejection rates of Naive and AHT tests, by study design and dimension of hypothesis ($q$). CR = cluster-randomized design; DD = difference-in-differences design; RB = randomized block design.}\label{fig:balance}
\end{sidewaysfigure}
...
% In particular, the AHT degrees of freedom are just round(AL_results[12,"df"],2), far lower than the number of clusters. 
...
For the artificial Hausman test the AHT test has fewer than 9 degrees of freedom, which leads to a much larger p-value compared to using the CR1 F test. 
