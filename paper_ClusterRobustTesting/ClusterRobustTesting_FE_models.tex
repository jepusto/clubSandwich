\documentclass[draft]{ectaart}\usepackage[]{graphicx}\usepackage[]{color}
...
\caption[Rejection rates of AHT and standard tests for ]{Rejection rates of AHT and standard tests for $\alpha = .05$, by dimension of hypothesis ($q$) and sample size ($m$).}\label{fig:overview}
\end{sidewaysfigure}
...
For example, for $\alpha = .05$, the maximum Type I error ranges from 0.123 ($q = 1$) to 0.682 ($q = 6$) for data sets with 15 clusters.
...
Again, focusing on the $\alpha = 0.05$ case, the maximum error ranges from 0.073 ($q = 1$) to 0.18 ($q = 6$).
...
\caption[Rejection rates of AHT and standard tests, by study design and dimension of hypothesis (]{Rejection rates of AHT and standard tests, by study design and dimension of hypothesis ($q$) for $\alpha = .05$ and $m = 30$. CR = cluster-randomized design; DD = difference-in-differences design; RB = randomized block design; B = balanced; U = unbalanced.}\label{fig:balance}
\end{sidewaysfigure}
