...
<<Angrist_Lavy, cache = TRUE, results = "asis">>=
library(xtable)

source("R/Angrist & Lavy example.R")
...
<<MLDA_table, cache = TRUE, results="asis">>=
library(xtable)
...
M2 <- as.data.frame(rbind(RE2_CR1, RE2_CR2, FE2_CR1, FE2_CR2, FE2_CR2A, Haus2_CR1, Haus2_CR2))
rownames(M2) <- NULL
M2$Correction <- c(c("CR1","CR2","CR2"),c("CR1","CR2","CR2","CR2","CR2"),c("CR1","CR2","CR2"))
M2$Test <- c(c("Standard","Standard","AHT"),c("Standard","Standard","AHT","Standard","AHT*"),c("Standard","Standard","AHT"))
M2$Hypothesis <- c("Random effects",NA,NA,"Fixed effects",NA,NA,NA,NA,"Hausman test",NA,NA)

filter(M2, Correction=="CR1" | Test %in% c("AHT","AHT*")) %>%
