...
<<Angrist_Lavy, cache = FALSE, results = "asis">>=
library(xtable)
...
<<MLDA_table, cache = FALSE, results="asis">>=
library(xtable)
...
For the artificial Hausman test, the AHT test has fewer than \Sexpr{round(Haus_CR2["HTZ","df"])} degrees of freedom, which leads to a much larger p-value compared to using the standard test based on CR1. 
