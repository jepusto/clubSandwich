...
Such correlation among units can arise from sampling aggregate units (e.g., countries, districts, villages), each of which contains multiple observations; from repeated measurement of an outcome on a common set of units, as in panel data; or from model mis-specification, as in analysis of regression discontinuity designs \citep[e.g.,][]{Lee2008regression}. 
...
The results of the simulations by \citet{Tipton2015small-F} indicate that the AHT F-test always has Type I error less than or equal to the stated $\alpha$ level, except in cases with extreme model mis-specification. 
...
The simulation study focused on testing hypotheses about the relative effects of three policy conditions, while varying the manner in which the policy indicators are assigned across clusters across three distinct designs. 
...
Thus, the simulation included a total of six distinct study designs, across which the performance of CRVEs is expected to vary. 
Appendix \ref{app:simulations} describes the exact specification of each design. For each design, we simulated designs with $m = 15$, 30, or 50 clusters, each with $n = 18$ or 30 units.

For a given study design, we simulated multivariate outcome data so that we could examine the performance of the proposed testing procedures for null hypotheses of varying dimension. 
Specifically, we simulated a tri-variate, equi-correlated outcome, based on a data-generating process in which all three policy conditions produce identical average outcomes, so that all tested null hypotheses hold. 
...
Further, we assumed that these random effects are correlated, which has the effect of inducing variability in the cluster-specific treatment effects and thus a degree of mis-specification into the analytic models described below. Letting $\sigma_\delta^2$ denote the degree of treatment effect variability relative to the total variability in a given outcome measurement, we simulated the random effects $\nu_{1i},\nu_{2i},\nu_{3i}$ to satisfy $\text{Var}\left(\nu_{gi} - \nu_{hi}\right) = \sigma_\delta^2$ for $g \neq h, g,h = 1,2,3$. 
...
Under this data-generating process, we simulated data based on parameter values of $\tau^2 = .05$, .15, or .25 for the intra-class correlation; outcome correlations that were either weak ($\rho = .2$) or strong ($\rho = .8$); and values of $\sigma_\delta^2 = .00$, .01, or .04 for treatment effect variability.\todo{Explanation of how these parameters affect misspecification of analytic models.} Each combination of sample sizes and parameter levels was simulated under each of the six study designs, yielding a total of $3 \times 2 \times 3 \times 2 \times 3 \times 6 = \Sexpr{3 * 2 * 3 * 2 * 3 * 6}$ simulation conditions.
...
The analytic model (\ref{eq:sim_analytic_model}) provided opportunities to test a range of single- and multi-parameter hypotheses. 
...
We then tested the multi-variate versions of the above tests, which involve all three outcome measures jointly. Here, we tested the null hypotheses that a given policy condition had no average effects on any outcome (i.e., $H_0: \mu_{11} = \mu_{1h}, \mu_{21} = \mu_{2h}, \mu_{31} = \mu_{3h}$, for $h = 2$ or $h = 3$) which has dimension $q = 3$. 
Finally, we tested the null hypothesis of no differences among policy conditions on any outcome ($H_0: \mu_{11} = \mu_{12} = \mu_{13}, \mu_{21} = \mu_{22} = \mu_{23}, \mu_{31} = \mu_{32} = \mu_{33}$), which has dimension $q = 6$. 

For a given combination of sample sizes, parameter levels, and study design, we simulated 10,000 datasets from model (\ref{eq:data_generating_model}), estimated model (\ref{eq:sim_analytic_model}) on each dataset, and tested all of the hypotheses described above. Simulated Type I error rates therefore have standard errors of approximately \Sexpr{round(sqrt(.01 * .99 / 10000),3)} for $\alpha = .01$, \Sexpr{round(sqrt(.05 * .95 / 10000),3)} for $\alpha = .05$, and \Sexpr{round(sqrt(.1 * .9 / 10000),3)} for $\alpha = .10$. 
...
filter(results_long_all, test %in% c("CR1 Naive-F", "CR2 HTZ") 
  ...
  within({
    test <- factor(ifelse(test=="CR1 Naive-F", "Naive", "AHT"), levels = c("Naive","AHT"))
...
<<overview, fig.width = 12, fig.height = 8.5, out.width = "\\linewidth", fig.cap = "Rejection rates of Naive and AHT tests, by dimension of hypothesis ($q$) and nominal type I error ($\\alpha$)", fig.env = "sidewaysfigure">>=
...
Figure \ref{fig:overview} shows that Naive-F = bad, AHT = rad! Also, performance of the naive test degrades (becomes liberal) for larger q, whereas AHT test becomes more conservative.

<<balance, fig.width = 12, fig.height = 8.5, out.width = "\\linewidth", fig.cap = "Rejection rates of Naive and AHT tests, by study design and dimension of hypothesis ($q$). CR = cluster-randomized design; DD = difference-in-differences design; RB = randomized block design.", fig.env = "sidewaysfigure">>=
...
Figure \ref{fig:misspecification} shows that model mis-specification doesn't matter for AHT. 
...
As in previous examples, our preferred version of the test uses the CR2S adjustment to the variance estimator and the AHT test. 
...
For the artificial Hausman test the AHT test has fewer than \Sexpr{round(Haus2_CR2["HTZ","df"])} degrees of freedom, which leads to a much larger p-value compared to using the CR1 F test. 
