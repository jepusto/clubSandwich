...
In the top row, rejection rates of the AHT test are near or below nominal (between \Sexpr{round(AHT_range_m30$min, 3)} and \Sexpr{round(AHT_range_m30$max, 3)}) across all conditions with at least 30 clusters.
