...
\end{equation}
In this model, time-point $t$ is nested within state $i$; the outcome $y_{it}$ is log-earnings, which are reported in 1999 dollars; $\bm{r}_{it}$ is a vector of demographic covariates specific to the time point, including years of education, age, the square of age, and an indicator for female; $\gamma_t$ is a fixed effect for time point $t$; and $\mu_i$ is an effect for state $i$. 
