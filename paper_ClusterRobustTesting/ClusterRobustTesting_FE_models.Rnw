...
When used with CR2, the Satterthwaite approximation has been shown to perform well across a variety of data generation processes, covariate types, and weighting procedures (see Section 4).\todo{Beth, possible to trim down the following two paragraphs?}
When the covariate is balanced across clusters---as occurs in a balanced panel with the same proportion of treated units in each cluster---the degrees of freedom are $m - 1$. 
