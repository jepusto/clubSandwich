...
\usepackage[textwidth=1in, textsize=tiny,disable]{todonotes}
...
\begin{frontmatter}
\title{Small sample methods for cluster-robust variance estimation and hypothesis testing in fixed effects models\protect\thanksref{T1}}
\runtitle{Cluster-robust variance estimation in fixed effects models}
\thankstext{T1}{The authors thank Dan Knopf for helpful discussions about the linear algebra behind the cluster-robust variance estimator.}

\author{James E. Pustejovsky}
\author{Elizabeth Tipton}
\runauthor{Pustejovsky \& Tipton}
...
\begin{abstract}
Cluster-robust variance estimation (CRVE) is commonly used to account for heteroskedasticity and un-modeled dependence among the errors in regression models. Although asymptotically consist, CRVE can be biased downward when the number of clusters is small, leading to hypothesis tests with overly liberal rejection rates. More accurate tests can be constructed using bias-reduced linearization (BRL), which corrects the CRVE based on a working model, in conjunction with t-tests with Satterthwaite degrees of freedom. We propose a generalization of BRL that can be applied in models with arbitrary sets of fixed effects, where the original BRL method is undefined. We also propose a small-sample test for multiple-parameter hypotheses, which generalizes the Satterthwaite approximation for t-tests. In simulations covering a wide range of scenarios, we find that conventional cluster-robust Wald tests can severely under-reject while the proposed small-sample test maintains Type I error close to nominal levels. 
\end{abstract}
...
In many economic analyses, interest centers on the parameters of a linear regression model, estimated by ordinary or weighted least squares (OLS/WLS) from a sample exhibiting within-group dependence. 
...
Here, each policy effect would be parameterized as a dummy variable in a regression model, which might also include other demographic controls. It is also common to include fixed effects for each state and each time-point in order to control for unobserved confounding in each dimension. 
...
Second, in models with fixed effects, the magnitude of the BRL adjustment depends on whether it is computed based on the full design matrix (i.e., the LSDV estimator) or after absorbing the fixed effects.\footnote{\citet{Cameron2015practitioners} noted that other methods of small-sample correction, including the correction used in Stata, suffer from the same problem.}  
...
Second, we demonstrate how to calculate the BRL adjustments based on the FE estimator and identify conditions under which first-stage absorption of the fixed effects can be ignored.
...
Section \ref{sec:examples} illustrates the use of the proposed hypothesis tests in two applications, drawn from contexts where CRVE is commonly used. 
...
In the state-policy example described in the introduction, $\bm{R}_i$ would include indicator variables for each policy change, as well as additional demographic controls; $\bm{S}_i$ would include year fixed effects; and $\bm{T}_i$ would indicate state fixed effects. 
...
\end{equation}
This estimator is algebraically identical to the LSDV estimator, $\bs{\hat\beta} = \bm{K}'\bm{M_X} \bm{X}' \bm{W} \bm{y}$, but avoids the need to solve a system of $p$ linear equations.
...
The form of these adjustment matrices parallels those of the heteroskedasticity-consistent variance estimators proposed by \citet*{MacKinnon1985some}. 
...
A more principled approach to bias correction would take into account these features of the covariates. 
...
In contrast to the CR1 or CR3 estimators, the BRL correction is premised on a ``working'' model for the structure of the errors, which must be specified by the analyst. 
...
For example, we might assume that the errors are uncorrelated and homoskedastic, with $\bs\Phi_i = \bm{I}_i$. 
...
This formulation of $\bm{A}_i$ is problematic because, for some fixed effects models that are common in economic applications, Equation \ref{eq:CR2_criterion_BM} does not have any solution. 
In the next two subsections, we address this and other issues that arise in applying BRL to fixed effects models.
...
\citet{Angrist2009mostly} note that this problem occurs in balanced state-by-year panel models that include fixed effects for states and for years. 
However, an alternative criterion for the adjustment matrices can always be satisfied. 
Instead of (\ref{eq:CR2_criterion_BM}), we seek adjustment matrices $\bm{A}_i$ that satisfy:
\begin{equation}
...
Criterion (\ref{eq:CR2_criterion}) does not uniquely define $\bm{A}_i$. One solution, which produces symmetric adjustment matrices, uses
\begin{equation}
...
A problem with existing small-sample adjustments to CRVEs is that they can result in a different estimator depending upon if the regression estimates are calculated using LSDV or after absorbing the fixed effects. 
...
\citet{Cameron2015practitioners} highlight that this discrepancy can be substantial if the clusters are small. For instance, if each cluster includes $n_i = 2$ units, then the CR1S estimator based on the LSDV estimator is over twice as large as the estimator based on the absorbed model.
...
Similar discrepancies can arise when applying the BRL method in models with fixed effects. 
...
However, this approach can lead to different adjustment matrices because it is based on a subtly different working model. 
...
In contrast, if the working model proposed by \citet{Imbens2015robust} is instead used (while still using OLS), then the above theorem implies that the CR2 adjustments will differ if the model is estimated by LSDV versus by using absorption.
...
Cluster-robust Wald-type test statistics, which are a function of the parameter estimates $\bs{\hat\beta}$ and the corresponding CRVE, are justified on an asymptotic basis as the number of clusters grows large (i.e., as $m \to \infty$). 
...
Alternately, for any of the CRVEs one could instead use an ``empirical'' estimate of the degrees of freedom, constructed by substituting $\bm{e}_i \bm{e}_i'$ in place of $\bs\Phi_i$. 
...
The Satterthwaite approximation works because the degrees of freedom account for covariate features that affect the distribution of the test statistic. When the covariate is balanced---as occurs in a balanced panel with the same proportion of treated units in each cluster---the degrees of freedom reduce to $m - 1$. 
...
Previous simulation studies have studied the performance of t-tests based on the CR2 variance estimator and Satterthwaite approximation under a variety of conditions, including panel data models \citep{Cameron2015practitioners, Imbens2015robust}, analysis of multi-stage surveys \citep{Bell2002bias}, and meta-analysis \citep{Tipton2015small-t}. Across this range of data-generating processes, these studies found that the Type I error rate of the test is nearly always less than or equal to the nominal $\alpha$, so long as the degrees of freedom are larger than 4 or 5 \citep{Bell2002bias, Tipton2015small-t}.
However, when the degrees of freedom are very small, the t-distribution approximation to the sampling distribution does not hold, and the Type I error can be higher than the stated $\alpha$ level. \footnote{When the degrees of freedom are smaller than 4 or 5, \citet{Tipton2015small-t} suggested using a smaller $\bs\alpha$ level for hypothesis testing in order to partially compensate. Another possibility would be to use the saddlepoint approximation proposed by \citet{McCaffrey2006improved}.}
...
Consequently, \citet{Tipton2015small-t} and \citet{Imbens2015robust} have argued that t-tests based on CRVE should routinely use the CR2 variance estimator and the Satterthwaite degrees of freedom, even when $m$ appears to be large.
...
\citet{Tipton2015small-F} recently introduced this test for the special case of CRVE in meta-regression models.
...
We expect that using the AHT degrees of freedom, which take into account features of the covariate distribution, will improve the accuracy of the rejection rates in small samples. 
...
Evidence from several large simulation studies indicates that hypothesis tests based on the CR2 adjustment and estimated degrees of freedom substantially out-perform the procedures that are most commonly used in empirical applications \citep{Cameron2015practitioners, Imbens2015robust,Bell2002bias,Tipton2015small-t,Tipton2015small-F}.
...
The simulation study focused on testing hypotheses about the relative effects of three policy conditions, based on each of three distinct study designs. 
...
For a given study design, we simulated tri-variate outcome data so that we could examine the performance of the proposed testing procedures for constraints of varying dimension. 
...
The random effects for unit $i$ are taken to follow a compound symmetric distribution with $\Var\left(\nu_{hi}\right) = \tau^2$ and $\text{Var}\left(\nu_{gi} - \nu_{hi}\right) = \sigma_\delta^2$ for $g \neq h, g,h = 1,2,3$. 
This has the effect of inducing variability in the cluster-specific treatment effects and thus a degree of misspecification into the working model. 
...
Each combination of sample sizes and parameter levels was simulated under each of the six study designs, yielding a total of \Sexpr{3 * 2 * 3 * 2 * 3 * 6} simulation conditions.
...
The general analytic model for the difference-in-differences design is
\begin{equation}
...
For the cluster-randomized designs, fixed effects for clusters are omitted because the clusters are nested within treatment conditions. For the randomized block designs, the fixed effects for time-points are omitted for simplicity.
The model is estimated by OLS after absorbing any fixed effects, and so the ``working'' model amounts to assuming that the errors are independent and identically distributed. Note that the working model departs from the true data generating model both because of correlation among the outcomes ($\rho > 0$) and because of treatment effect variability ($\sigma_\delta^2 > 0$). The range of parameter combinations used in the true data generating model thus allows us to examine the performance of the AHT test under both small and large degrees of working model misspecification. 

Analytic model (\ref{eq:sim_analytic_model}) provides opportunities to test several single- and multi-parameter constraints. 
...
We then tested the multi-variate versions of the above tests, which involve all three outcome measures jointly; the multi-variate versions of the tests thus have dimension $q = 3$ ($H_0: \mu_{11} = \mu_{1h}, \mu_{21} = \mu_{2h}, \mu_{31} = \mu_{3h}$, for $h = 2$ or $h = 3$) or $q = 6$ ($H_0: \mu_{11} = \mu_{12} = \mu_{13}, \mu_{21} = \mu_{22} = \mu_{23}, \mu_{31} = \mu_{32} = \mu_{33}$). 
...
Simulated Type I error rates therefore have standard errors of approximately \Sexpr{round(sqrt(.01 * .99 / 10000),4)} for $\alpha = .01$, \Sexpr{round(sqrt(.05 * .95 / 10000),4)} for $\alpha = .05$, and \Sexpr{round(sqrt(.1 * .9 / 10000),4)} for $\alpha = .10$. 

\subsection{Simulation Results}

Our discussion of the simulation results is focused on four trends. Throughout, we present results for the nominal Type I error rate of $\alpha = .05$; figures presenting results for $\alpha = .01$ and $\alpha = .10$ are available in the supplementary materials, along with complete numerical results and replication code.
...
<<overview, fig.width = 10, fig.height = 5, out.width = "\\linewidth", fig.cap = "Rejection rates of AHT and standard tests for $\\alpha = .05$, by dimension of hypothesis ($q$) and sample size ($m$).", fig.env = "sidewaysfigure">>=
...
filter(results_long, m == 15 & test == "AHT") %>%
  group_by(alpha) %>%
...
In this and following figures, the solid horizontal line indicates the stated $\alpha$ level and the dashed line indicates an upper confidence bound on simulation error.
...
Although there exist situations in which the error is above the simulation bound, the departures are typically small. For example, when $m = 15$ the rejection rates do not exceed \Sexpr{round(AHT_max_reject[1,"reject"],3)} for $\alpha = .01$, \Sexpr{round(AHT_max_reject[2,"reject"],3)} for $\alpha = .05$, and \Sexpr{round(AHT_max_reject[3,"reject"],3)} for $\alpha = .10$.
...
For example, for $\alpha = .05$, the maximum Type I error ranges from \Sexpr{round(standard_max_reject[1,"15"],3)} ($q = 1$) to \Sexpr{round(standard_max_reject[4,"15"],3)} ($q = 6$) for data sets with 15 clusters.
...
Again, focusing on the $\alpha = 0.05$ case, the maximum error ranges from \Sexpr{round(standard_max_reject[1,"50"],3)} ($q = 1$) to \Sexpr{round(standard_max_reject[4,"50"],3)} ($q = 6$).

<<balance, fig.width = 10, fig.height = 5.5, out.width = "\\linewidth", fig.cap = "Rejection rates of AHT and standard tests, by study design and dimension of hypothesis ($q$) for $\\alpha = .05$ and $m = 30$. CR = cluster-randomized design; DD = difference-in-differences design; RB = randomized block design; B = balanced; U = unbalanced.", fig.env = "sidewaysfigure">>=
...
filter(results_long, m == 30 & test == "AHT" & alpha == .05) %>%
  summarise(min = min(reject), max = max(reject)) ->
  AHT_range_m30

@

In order to better understand the effects of different parameter combinations on the performance of both tests, Figure \ref{fig:balance} breaks out the results by study design for the sample size of $m = 30$. 
...
For studies with at least 30 clusters, rejection rates of the AHT test are very closes to nominal (between \Sexpr{round(AHT_range_m30$min, 3)} and \Sexpr{round(AHT_range_m30$max, 3)}) across all conditions studied.
...
These findings follow closely those from \citet{Tipton2015small-F}, which also found that the Type I error of the AHT test was close to nominal even when the working model was very discrepant from the data-generating model.

<<df, fig.width = 8, fig.height = 3, out.width = "\\linewidth", fig.cap = "Range of denominator degrees of freedom for AHT test, by number of clusters and study design. CR = cluster-randomized design; DD = difference-in-differences design; RB = randomized block design; B = balanced; U = unbalanced.">>=
...
Data files and replication code (in R) are available in the supplementary materials.
...
panel_fit <- plm(zakaibag ~ yr2000:school_type + yr2001:school_type + 
             ...
             data = panel_dat, index = c("school_id","student_id"))
...
t_CR2 <- coef_test(panel_fit, vcov="CR2", test = c("naive-t","Satterthwaite"))[panel_constraints,]

joint_CR1 <- Wald_test(panel_fit, constraints = panel_constraints, vcov = "CR1", test = "Naive-F")
joint_CR2 <- Wald_test(panel_fit, constraints = panel_constraints, vcov = "CR2", test = c("Naive-F","HTZ"))
...
mod_CR2 <- Wald_test(panel_school, constraint_list, vcov = "CR2", test = c("Naive-F","HTZ"))
@

<<AL_ATEs, results = "asis">>=
t_CR1 <- within(t_CR1, {
  df <- 34
...
t_tests <- as.data.frame(rbind(select(t_CR1, hypothesis, CR, test, beta, SE, df, p_val),
                               select(t_CR2A, hypothesis, CR, test, beta, SE, df, p_val), 
                               select(t_CR2B, hypothesis, CR, test, beta, SE, df, p_val)))

t_tests$Fstat <- with(t_tests, (beta / SE)^2)
            
      
joint_CR1_df <- within(joint_CR1, {
  hypothesis <- "Joint"
  test <- rownames(joint_CR1)
  CR <- "CR1"
})
joint_CR2_df <- within(joint_CR2, {
  hypothesis <- "Joint"
  test <- rownames(joint_CR2)
  CR <- "CR2"
})

mod_CR1_df <- within(bind_rows(mod_CR1), {
  hypothesis <- paste("Moderation", names(mod_CR1), sep = " - ")
  test <- rownames(mod_CR1[[1]])
  CR <- "CR1"
})
mod_CR2_df <- within(bind_rows(mod_CR2), {
  hypothesis <- paste("Moderation", names(mod_CR2), sep = " - ")
  test <- rownames(mod_CR2[[1]])
  CR <- "CR2"
})

AL_results <- rbind(select(t_tests, Hypothesis = hypothesis, Correction = CR, Test = test, F = Fstat, df, p = p_val), 
                 select(bind_rows(joint_CR1_df, joint_CR2_df, mod_CR1_df, mod_CR2_df), 
                        Hypothesis = hypothesis, Correction = CR, Test = test, F = Fstat, df, p = p_val))
rownames(AL_results) <- NULL

AL_results <- within(AL_results, {
  Hypothesis <- factor(Hypothesis, 
                             levels = c("treated2001:half1","treated2001:half2","Joint",
                                        paste("Moderation", c("lower","upper","joint"), sep = " - ")),
                             labels = c("ATE - lower half (q = 1)", "ATE - upper half (q = 1)", 
                                        "ATE - joint (q = 2)", "Moderation - lower half (q = 2)", 
                                        "Moderation - upper half (q = 2)", "Moderation - joint (q = 4)"))
  Test <- factor(Test, levels = c("Naive-F", "HTZ"), labels = c("Standard","AHT"))
  df <- round(df, 2)
  p <- round(p, 5)
})

arrange(AL_results, Hypothesis, Correction, Test) %>%
  filter(Correction=="CR1" | Test=="AHT", !grepl("lower", Hypothesis)) %>%
  select(-Correction) ->
  AL_results
...
For the first two hypotheses, the AHT test statistics are only slightly smaller than their standard counterparts, but the degrees of freedom are considerably smaller. 
...
RE2_Hausman <- lme(mrate ~ legal + beertaxa + legal_s + beertaxa_s + factor(year), data = death_dat, random = ~ 1 | state)
...
For the artificial Hausman test, the AHT test has fewer than \Sexpr{round(Haus2_CR2["HTZ","df"])} degrees of freedom, which leads to a much larger p-value compared to using the standard test based on CR1. 

\section{Conclusion}
\label{sec:conclusion}

Across the discipline of economics, empirical studies often involve modeling data with a correlated error structure. 
...
In this paper, we have extended the CR2 variance estimator so that it is a fully general solution, which can be applied even in models with fixed effects in multiple dimensions. 
...
Furthermore, our empirical examples illustrate that the degrees of freedom adjustment can have a relatively large influence on small-sample performance.
...
\label{eq:step1}
\bm{\ddot{R}}_i' \bm{W}_i \bm{A}_i \left(\bm{I} - \bm{H_X}\right)_i \bs\Phi \left(\bm{I} - \bm{H_X}\right)_i' &\bm{A}_i' \bm{W}_i \bm{\ddot{R}}_i \\
&= \bm{\ddot{R}}_i' \bm{W}_i \bm{D}_i \bm{B}_i^{+1/2} \bm{B}_i \bm{B}_i^{+1/2} \bm{D}_i' \bm{W}_i \bm{\ddot{R}}_i \\
...
\begin{align*}
&\left(\bs\Phi_i - \bm{\ddot{U}}_i \bm{M_{\ddot{U}}}\bm{\ddot{U}}_i' - \bm{T}_i \bm{M_T}\bm{T}_i'\right)^+ \\
&\qquad \qquad \qquad = \bs\Psi_i + \bs\Psi_i \bm{T}_i \bm{M_T}\left(\bm{M_T} - \bm{M_T}\bm{T}_i' \bs\Psi_i \bm{T}_i \bm{M_T}\right)^+ \bm{M_T} \bm{T}_i' \bs\Psi_i \\
&\qquad \qquad \qquad = \bs\Psi_i + \bm{W}_i \bm{T}_i \bm{M_T}\left(\bm{M_T} - \bm{M_T}\bm{T}_i' \bm{W}_i \bm{T}_i\bm{M_T}\right)^+ \bm{M_T} \bm{T}_i' \bm{W}_i \\
&\qquad \qquad \qquad = \bs\Psi_i.
\end{align*}
