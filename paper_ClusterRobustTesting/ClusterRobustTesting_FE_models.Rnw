...
<<STAR_results, results="asis">>=
load("STAR_test_results.Rdata")
filter(F_tests, grepl("_plm", .id)) %>%
  mutate(Outcome = factor(.id, levels = c("read_plm","math_plm","word_plm","SUR_plm"),
                          labels = c("Reading","Math","Word recognition","Combined (SUR)")),
         Correction = paste0("CR",CR),
         Test = factor(test, levels = c("Naive-F","HTZ"), labels = c("F","AHZ")),
         F = round(Fstat,3),
         df = round(df, 2),
         p = round(p_val, 5)) %>%
  select(Outcome, Correction, Test, F, df, p) ->
  STAR_results

STAR_results$Outcome[-seq(1,12,3)] <- NA
...
mod_CR1 <- Wald_test(panel_school, constraints = school_constraints, vcov = "CR1", test = "Naive-F")
mod_CR2 <- Wald_test(panel_school, constraints = school_constraints, vcov = "CR2", test = c("Naive-F","HTZ"))
@
...
t_tests$F <- with(t_tests, (beta / SE)^2)
            
      
joint_CR1 <- within(joint_CR1, {
  hypothesis <- "Joint"
  test <- rownames(joint_CR1)
  CR <- "CR1"
})
joint_CR2 <- within(joint_CR2, {
  hypothesis <- "Joint"
  test <- rownames(joint_CR2)
  CR <- "CR2"
})

mod_CR1 <- within(mod_CR1, {
  hypothesis <- "Moderation"
  test <- rownames(mod_CR1)
  CR <- "CR1"
})
mod_CR2 <- within(mod_CR2, {
  hypothesis <- "Moderation"
  test <- rownames(mod_CR2)
  CR <- "CR2"
})

AL_results <- rbind(select(t_tests, Hypothesis = hypothesis, Correction = CR, Test = test, 8, df, p = p_val), 
                 select(rbind(joint_CR1, joint_CR2, mod_CR1, mod_CR2), 
                        Hypothesis = hypothesis, Correction = CR, Test = test, F = Fstat, df, p = p_val))
AL_results <- within(AL_results, {
  Hypothesis <- factor(Hypothesis, 
                             levels = c("treated2001:half1","treated2001:half2","Joint","Moderation"),
                             labels = c("ATE - lower half (q = 1)", "ATE - upper half (q = 1)", 
                                        "ATE - joint (q = 2)", "Moderation by sector (q = 4)"))
  Test <- factor(Test, levels = c("Naive-F", "HTZ"), labels = c("F","AHZ"))
  df <- round(df, 2)
  p <- round(p, 5)
})

AL_results <- arrange(AL_results, Hypothesis, Correction, Test) 
...
% In particular, the AHZ degrees of freedom are just \Sexpr{round(AL_results[12,"df"],2)}, far lower than the number of clusters. 
...
rownames(M2) <- NULL
MLDA_results <- cbind(select(M1, Hypothesis, Correction, Test, Fstat, df, p_val), NA, select(M2, Fstat, df, p_val))

print(xtable(MLDA_results,digits = c(0,0,0,0,3,2,5,0,3,2,5)), 
...
For the artificial Hausman test in the model that controls for beer taxes, the AHZ test has less than \Sexpr{round(Haus2_CR2["HTZ","df"])} degrees of freedom, which leads to a much larger p-value compared to using an F test with $m - 1 = 49$ degrees of freedom. 
