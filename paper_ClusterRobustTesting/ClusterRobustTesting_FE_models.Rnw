...
The proposed methods are implemented in the R package \texttt{clubSandwich}, which is available on the Comprehensive R Archive Network.\todo{Cite clubSandwich}
...
In combination with the study design factors, the simulation therefore included a total of \Sexpr{3 * 2 * 2 * 3 * 3 * 3 * 2 * 3} unique conditions.
...
When the number of clusters is very small and the dimension of the test is large, the Type I error can be much smaller than $\alpha$. 
