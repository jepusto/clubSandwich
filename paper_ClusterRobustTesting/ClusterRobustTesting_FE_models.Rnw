...
We posit the following following analytic model: 
\begin{equation}
...
First, using only the data for outcome $k$, we test the effects of the small class size ($H_0: \beta_{1k} = 0$) while maintaining the assumption that the additional classroom aide has no effect on student achievement (i.e., constraining $\beta_{2k} = 0$). 
...
Third, combining the data across all three outcomes, we test the hypothesis that small class size had no effects on any outcome (i.e., $\beta_{11} = \beta_{12} = \beta_{13} = 0$), while again maintaining the assumption that the additional classroom aide had no effects. 
...
In all models, we estimated $\bs\beta_k$ and $\bs\gamma$ after absorbing the school fixed effects and clustered the errors by school.
...
filter(STAR_F_tests, grepl("(math|SUR)", .id) & (CR==1 | test=="HTZ")) %>%
  mutate(Outcome = ifelse(grepl("math", .id), "Math","Combined"),
         ...
         Test = factor(test, levels = c("Naive-F","HTZ"), labels = c("Standard","AHZ")),
         F = round(Fstat,3),
         df = round(df, 2),
         p = round(p_val, 5)) %>%
...
Table \ref{tab:STAR} displays the results for a representative subset of these hypothesis tests, using either the standard hypothesis test or the AHZ test.
...
Student-level covariate and outcome data were drawn from administrative records available for the school years ending in June of 2000, 2001, and 2002. The primary outcome of interest is Bagrut completion. 
...
Letting $h = 1,2,3$ index the sector of each school (Arab religious, Jewish religious, or Jewish secular), we posit the following analytic model: 
\begin{equation}
\label{eq:AL_ATE}
...
Based on Model (\ref{eq:AL_ATE}), we test three hypotheses. 
First, we assume that the program effects are constant across sector (i.e., $\bs\beta_1 = \bs\beta_2 = \bs\beta_3 = \bs\beta$) and test for whether the program affected completion rates for students in each half of the prior achievement distribution (i.e., $H_0: \bs\beta = 0$).
Second, we test for whether treatment effects differ . 
