...
For each model, we first report an artificial-Hausman test \citep{Arellano1993on, Wooldridge2002econometric} to determine whether to use a fixed effects (FE) estimator or a random effects (RE) estimator for $\bs\beta$. 
...
The parameter $\bs\delta$ then captures the difference between the between-panel and within-panel estimates of $\bs\beta$. 
With this setup, the artificial Hausmann test amounts to testing the null hypothesis that $\bs\delta = \bm{0}$. 
We estimate the model using WLS with weights derived under the assumption that  $\mu_1,...,\mu_m$ are mutually independent, normally distributed, and independent of $\epsilon_{it}$.
...
coef_test(RE1, vcov = "CR0", test = "naive-t")
coef_test(RE1, vcov = "CR2", test = c("naive-t","Satterthwaite"))
