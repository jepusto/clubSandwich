...
Other recent work has proposed methods for cluster-robust inference with a small number of clusters. 
\citet{Cameron2008bootstrap} investigated a range of bootstrapping procedures that provide improved Type-I error control in small samples. 
...
\todo[inline]{Connect this to the IK case.}
