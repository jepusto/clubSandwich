...
For example, the maximum Type I error ranges from \Sexpr{round(standard_max_reject[1,"15"],3)} ($q = 1$) to \Sexpr{round(standard_max_reject[4,"15"],3)} ($q = 6$) for data sets with 15 clusters.
Perhaps even more important for practice, the rejection rate of the standard test can be far above the stated $\alpha$ level even when there are 50 clusters, with maximum error ranging from \Sexpr{round(standard_max_reject[1,"50"],3)} ($q = 1$) to \Sexpr{round(standard_max_reject[4,"50"],3)} ($q = 6$).
