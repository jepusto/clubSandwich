...
In an extensive simulation, \maskcitet{Tipton2015small-F} compared the performance of the AHT test to several other possible approximate F-tests, including adaptations of the tests introduced by \citet{Pan2002small} and \citet{Zhang2012twowayANOVA}, as well as adaptations of tests based on eigen-decompositions proposed by \citep{Fai1996approximate} and \citep{Cai2008new}.
Simulation results indicated that the AHT test presented here has Type I error closer to nominal than any of the other tests under a wide range of parameter values, covariate types, and hypotheses.
