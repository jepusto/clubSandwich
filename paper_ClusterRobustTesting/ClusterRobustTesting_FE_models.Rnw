...
In contrast, if the working model proposed by \citet{Imbens2015robust} is instead used (while still using OLS), then the above theorem implies that the CR2 adjustments might differ depending on whether LSDV or the fixed effects estimator is used.
...
Each combination of sample sizes and parameter levels was simulated for balanced and unbalanced versions of each of the three study designs, yielding a total of \Sexpr{3 * 2 * 3 * 2 * 3 * 6} simulation conditions.
...
Although there exist situations in which the error is above the simulation bound, the departures are typically small. For example, when $m = 15$ the rejection rates do not exceed \Sexpr{fmt(AHT_max_reject[1,"reject"],3)} for $\alpha = .01$, \Sexpr{fmt(AHT_max_reject[2,"reject"],3)} for $\alpha = .05$, and \Sexpr{fmt(AHT_max_reject[3,"reject"],3)} for $\alpha = .10$.
