...
Throughout, we refer to tests employing estimated degrees of freedom as ``AHT'' tests; for t-tests, the estimated degrees of freedom are equivalent to the Satterthwaite approximation given in Equation (\ref{eq:nu_model}). 
We refer to tests employing $m - 1$ degrees of freedom as ``ad hoc'' tests. 
...
the supplementary materials provide the exact specifications of each design. For each design, we simulated studies with $m = 15$, 30, or 50 clusters, each with $n = 18$ or 30 units. Table \ref{tab:simulation_parameters} summarizes the design of the simulation study.
...
Under this data-generating process, we simulated three different levels of intra-class correlation; three levels of treatment effect variability ($\sigma_\delta^2$), and outcomes that were either weakly ($\rho = .2$) or strongly correlated ($\rho = .8$).
Each combination of sample sizes and parameter levels was simulated for balanced and unbalanced versions of each of the three study designs, yielding a total of \Sexpr{3 * 2 * 3 * 2 * 3 * 6} unique conditions.
...
The range of parameter combinations used in the true data generating model thus allows us to examine the performance of AHT tests under both small and large degrees of working model misspecification. 
...
For a given combination of sample sizes, parameter levels, and study design, we simulated \Sexpr{format(iterations, big.mark = ",")} datasets from model (\ref{eq:data_generating_model}), estimated model (\ref{eq:sim_analytic_model}) on each dataset, and computed all of the hypothesis tests. 
Simulated Type I error rates therefore have standard errors of approximately \Sexpr{fmt(sqrt(.01 * .99 / iterations))} for $\alpha = .01$, \Sexpr{fmt(sqrt(.05 * .95 / iterations))} for $\alpha = .05$, and \Sexpr{fmt(sqrt(.1 * .9 / iterations))} for $\alpha = .10$. 

\subsection{Simulation Results}

<<overview, fig.width = 8, fig.height = 9.5, out.width = "\\linewidth", fig.cap = "Rejection rates of ad hoc and AHT tests for $\\alpha = .05$, by dimension of hypothesis ($q$) and sample size ($m$). The solid horizontal line indicates the stated $\\alpha$ level and the dashed line indicates an upper confidence bound on simulation error.">>=
...
Throughout, we present results for the nominal Type I error rate of $\alpha = .05$; figures presenting results for other $\alpha$ levels are available in the supplementary materials, along with complete numerical results and replication code.

The first finding is that the AHT test based on the CR2 estimator has Type I error close to the stated $\alpha$ level for all parameter combinations studied, whereas the standard, ad hoc test based on CR1 does not. 
Figure \ref{fig:overview} illustrates this pattern at the nominal type I error rate of $\alpha = .05$, for constraints of varying dimension (from $q = 1$, in the first row, to $q = 6$, in the final row) and varying number of clusters.
...
Although there exist situations in which the error is above the simulation bound, the departures are typically small. For example, when $m = 15$ the rejection rates do not exceed \Sexpr{fmt(AHT_max_reject[2,"reject"],3)} for $\alpha = .01$, \Sexpr{fmt(AHT_max_reject[3,"reject"],3)} for $\alpha = .05$, and \Sexpr{fmt(AHT_max_reject[4,"reject"],3)} for $\alpha = .10$.
The rejection rates are even closer to nominal for lower-dimensional constraints.

In comparison to the AHT test, the Type I error for the ad hoc test based on CR1 can be markedly higher than the stated $\alpha$ level, particularly when the number of clusters is small or the dimension of the hypothesis is large. 
For example, the maximum Type I error ranges from \Sexpr{fmt(standard_max_reject[1,"15"],3)} ($q = 1$) to \Sexpr{fmt(standard_max_reject[4,"15"],3)} ($q = 6$) for data sets with 15 clusters.
Perhaps even more important for practice, the rejection rate of the ad hoc test can be far above the stated $\alpha$ level even when there are 50 clusters, with maximum error ranging from \Sexpr{fmt(standard_max_reject[1,"50"],3)} ($q = 1$) to \Sexpr{fmt(standard_max_reject[4,"50"],3)} ($q = 6$). 
Using the AHT degrees of freedom with the CR1 variance estimator (middle row of Figure \ref{fig:overview}) leads to a drastic improvement, although the rejection rates still exceed the nominal level under some conditions.\todo{Comment on absorption-only adjustments.} 

<<balance, fig.width = 8, fig.height = 10, out.width = "\\linewidth", fig.cap = "Rejection rates of AHT and standard tests, by study design and dimension of hypothesis ($q$) for $\\alpha = .05$ and $m = 30$. The solid horizontal line indicates the stated $\\alpha$ level and the dashed line indicates an upper confidence bound on simulation error. CR = cluster-randomized design; DD = difference-in-differences design; RB = randomized block design; B = balanced; U = unbalanced.">>=
...
In the left-most column, it can be seen that the rejection rate of the ad hoc test increases with the dimension of the test ($q$) and the degree of unbalance in the study design.
...
The middle and right columns of Figure \ref{fig:balance} displays results for the AHT test with the CR1 and CR2 estimators, respectively.
For studies with at least 30 clusters, rejection rates of the AHT test with CR2 are very close to nominal (between \Sexpr{round(AHT_range_m30$min, 3)} and \Sexpr{round(AHT_range_m30$max, 3)}) across all conditions studied.
...
The rejection rates of the AHT test with the CR1 variance estimator sometimes exceeds the nominal levels, particularly for cluster-randomized designs.
At the smallest sample size, the rejection rates of the AHT tests are close to 0 in unbalanced CR and DD designs (see supplementary figures S4-S7). 
...
Across the nine error structures, the range of rejection rates remains very similar for the AHT test, with no clear pattern related to the degree of mis-specification (see supplementary figures S16-S19). 
These findings follow closely those from \maskcitet{Tipton2015small-F}, which also found that the Type I error of the AHT test was close to nominal even when the working model was quite discrepant from the data-generating model.

<<df, fig.width = 8, fig.height = 3.3, out.width = "\\linewidth", fig.cap = "Range of denominator degrees of freedom for AHT test, by number of clusters and study design. CR = cluster-randomized design; DD = difference-in-differences design; RB = randomized block design; B = balanced; U = unbalanced.">>=
...
Finally, Figure \ref{fig:df} depicts the range of estimated degrees of freedom for the AHT test (with CR2) as a function of the study design and number of clusters ($m$). 
...
Compared to using the AHT test with the CR1 variance estimator, using the AHT with CR2 leads to rejection rates that are closer to maintaining the nominal level, although the differences are often fairly small. 
An important question that remains is how much the standard, ad hoc test and AHT test diverge in actual application.
...
This article has benefited from the feedback of seminar participants at New York University, the University of Texas Population Research Center, and the American Institutes for Research, as well as an associate editor and referees.\todo{Beth, anything to add?} The authors thank Dan Knopf for helpful discussions about the linear algebra behind the cluster-robust variance estimator.
