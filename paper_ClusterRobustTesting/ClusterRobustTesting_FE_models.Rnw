...
Second, we test for whether program effects differ from zero in each half of the prior academic performance ($H_0: \bs\beta = 0$, with $q = 2$), still assuming that program effects are constant across sector. 
...
Finally, we conduct a joint test for whether program effects in each half of the prior achievement distribution vary by school sector ($H_0: \bs\beta_1 = \bs\beta_2 = \bs\beta_3$, with $q = 4$). 
