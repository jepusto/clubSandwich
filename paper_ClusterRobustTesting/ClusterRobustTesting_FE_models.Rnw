...
\maskcitet{Tipton2015small-F} recently introduced this test for the special case of CRVE in regression models in used in meta-analysis.
...
\maskcitet{Tipton2015small-F} developed five tests and compared them in a large simulation study. 
These tests include versions of those introduced by \citet{Pan2002small} and \citet{Zhang2012twowayANOVA}, as well as eigen-decomposition approaches (extensions to methods provided in Fai & Cornelius, 1996; Cai & Hayes, 2008).
They show that the AHT test presented here has Type I error closer to nominal than any of the other tests under a wide range of parameter values, covariate types, and tests.
...
the supplementary materials provide the exact specifications used. Table \ref{tab:simulation_parameters} summarizes the design of the simulation study, which included a total of \Sexpr{3 * 2 * 3 * 2 * 3 * 6} unique conditions.
...
For a given combination of study design, sample sizes, and parameter values, we simulated \Sexpr{format(iterations, big.mark = ",")} datasets from model (\ref{eq:data_generating_model}), estimated model (\ref{eq:sim_analytic_model}) on each dataset, and computed all of the hypothesis tests. 
...
Perhaps even more important for practice, the rejection rate of the standard test can be far above the stated $\alpha$ level even when there are 50 clusters, with maximum error ranging from \Sexpr{fmt(standard_max_reject[1,"50"],3)} ($q = 1$) to \Sexpr{fmt(standard_max_reject[4,"50"],3)} ($q = 6$). 
...
In the top row, rejection rates of the AHT test are very close to nominal (between \Sexpr{round(AHT_range_m30$min, 3)} and \Sexpr{round(AHT_range_m30$max, 3)}) across all conditions with at least 30 clusters.
...
These findings follow closely those from \maskcitet{Tipton2015small-F}, which indicated that the Type I error of the AHT test was close to nominal even when the working model was quite discrepant from the data-generating model.
...
The model also includes fixed effects $\gamma_{ht}$ for each sector in each year and $\mu_{i}$ for each school. 
...
add_to_row <- paste0("\\bottomrule\n",
                     "\\multicolumn{5}{l}",
                     "{\\small{* AHT test calculated without accounting for absorption of fixed effects}}\n")
...
print(xtable(AL_results, 
      ...
      table.placement = "bth",
      add.to.row = list(pos = list(nrow(AL_results)), command = add_to_row))
@
...
In light of the simulation results, we would suggest that the test that accounts for absorption should be prefered in this application.\todo{Do we believe this?} 
...
print(xtable(MLDA_results, 
      ...
      table.placement = "bth", 
      caption.placement = "top",
      add.to.row = list(pos = list(nrow(MLDA_results)), command = add_to_row))
