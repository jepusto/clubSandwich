...
Finally, while most previous studies focused on OLS estimation, one study \citep{Tipton2015small-t} examined the performance of t-tests based on WLS estimation.\todo{Beth, is there any way to condense the four columns under ``Variable type''?}\todo{Beth, what is the difference between IK2015 Table 1 vs Table 2?}
...
In comparison, the Type I error of the standard test were often very high, with maximum rejection rates ranging from .17 to .22, depending on the dimension of the constraint being tested.
...
<<overview, fig.width = 12, fig.height = 8.5, out.width = "\\linewidth", fig.cap = "Rejection rates of Naive and AHZ tests, by dimension of hypothesis ($q$) and nominal type I error ($\\alpha$)", fig.env = "sidewaysfigure">>=
...
<<balance, fig.width = 12, fig.height = 8.5, out.width = "\\linewidth", fig.cap = "Rejection rates of Naive and AHZ tests, by study design and dimension of hypothesis ($q$). CR = cluster-randomized design; DD = difference-in-differences design; RB = randomized block design.", fig.env = "sidewaysfigure">>=
...
<<misspecification, fig.width = 12, fig.height = 8.5, out.width = "\\linewidth", fig.cap = "Rejection rates of AHZ test, by treatment effect variance and intra-class correlation.", fig.env = "sidewaysfigure">>=
...
arrange(AL_results, Hypothesis, Correction, Test) %>%
  filter(Correction=="CR1" | Test=="AHZ", !grepl("lower", Hypothesis)) %>%
  select(-Correction) ->
  AL_results
...
For the artificial Hausman test the AHZ test has fewer than \Sexpr{round(Haus2_CR2["HTZ","df"])} degrees of freedom, which leads to a much larger p-value compared to using the CR1 F test. 
