...
the supplementary materials provide the exact specifications used. Table \ref{tab:simulation_parameters} summarizes the design of the simulation study, which included a total of \Sexpr{3 * 2 * 3 * 3 * 3 * 2 * 3} unique conditions.
