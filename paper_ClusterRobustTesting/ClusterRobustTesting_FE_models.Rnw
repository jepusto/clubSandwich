...
Nonetheless, we find it more coherent to specify a working model in terms of the full regression, and thus prefer this approach when it is computationally feasible to do so. 
We investigate the differences between the approaches as part of the simulation study in Section \ref{sec:simulation}.\todo{Beth does this work for answering the reviewer's point?}
...
In an extensive simulation, \maskcitet{Tipton2015small-F} compared the performance of the AHT test to several other possible approximate F-tests, including adaptations of the tests introduced by \citet{Pan2002small} and \citet{Zhang2012twowayANOVA}, and several others.
...
In combination with the study design factors, the simulation therefore included a total of \Sexpr{3 * 2 * 2 * 3 * 3 * 3 * 2 * 3} unique conditions.\todo{Beth can you look this over to make sure it's clear?}
...
The range of parameter combinations used in the true data generating model thus allows us to examine the performance of AHT tests under varying degrees of working model misspecification.\todo{This too?} 
...
Simulated Type I error rates therefore have standard errors of approximately \Sexpr{fmt(sqrt(.05 * .95 / iterations))} for $\alpha = .05$. 
...
In the top row, rejection rates of the AHT test are at or below nominal (between \Sexpr{round(AHT_range_m30$min, 3)} and \Sexpr{round(AHT_range_m30$max, 3)}) across all conditions with at least 30 clusters.
...
Figures S16-S19 in the supplementary materials depicts the rejection rates of two variants of the AHT test, including the test based on the CR1 adjustment matrices and the test based on CR2, but without accounting for absorption of fixed effects. 
...
An important question that remains is how much the standard test and AHT test diverge in actual application.\todo{Beth anything we should add here?}
...
Even when used with the CR1 adjustment matrices, the degrees of freedom adjustment leads to much more accurate rejection rates, although using the CR2 estimator appears to be necessary to fully maintain the nominal level of the test. 
