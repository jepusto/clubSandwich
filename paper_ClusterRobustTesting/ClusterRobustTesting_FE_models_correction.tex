...
\hypersetup{
  pdftitle={Corrigendum: Small sample methods for cluster-robust variance estimation and hypothesis testing in fixed effects models},
  hidelinks,
...
\(\tilde{\mathbf{A}}_i \mathbf{T}_i = \mathbf{T}_i\). Denote the rank of
\(\mathbf{\ddot{U}}_i\) as \(u_i \leq r + s\) and take the thin QR
...
\(\mathbf{Q}_i'\mathbf{T}_i = \mathbf{0}\). Now observe that
...
\] and it can be seen that \begin{equation}
...
\(\tilde{\mathbf{A}}_i \mathbf{T}_i = \tilde{\mathbf{B}}_i^{+1/2} \mathbf{T}_i = \mathbf{T}_i\).
