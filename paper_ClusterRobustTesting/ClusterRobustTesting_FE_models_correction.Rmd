...
From the fact that $\bm{\ddot{U}}_i'\bm{T}_i = \bm{0}$ for $i = 1,...,m$, it follows that
\begin{align}
...
\end{equation}
The conditions of Lemma \ref{lemma} hold for the inner term in (\ref{eq:Btilde_i}) because we have assumed that $\bm{L}_i = \bm{M}_\bm{\ddot{U}}^{-1} - \bm{\ddot{U}}_i' \bm{\ddot{U}}_i$ is non-singular. Thus, 
\begin{align}
\tilde{\bm{B}}_i^+ &= \left(\bm{I}_i - \bm{\ddot{U}}_i \bm{M_{\ddot{U}}}\bm{\ddot{U}}_i'\right)^+ \nonumber \\
&= \left(\bm{I}_i + \bm{\ddot{U}}_i \bm{L}_i^{-1} \bm{\ddot{U}}_i'\right).
\end{align}
Next, let $\bm{Z}_i = \bm{I}_i - \bm{T}_i \bm{M_T}\bm{T}_i'$. Because $\bm{T}_i$ consists of cluster-specific predictors, $\bm{T}_i \bm{M_T} \bm{T}_i' = \bm{T}_i \left(\bm{T}_i'\bm{T}_i\right)^{-1} \bm{T}_i'$. It follows that $\bm{Z}_i$ is idempotent, with $\bm{Z}_i^+ = \bm{Z}_i$. Further, $\bm{Z}_i \bm{Z}_i^+ \bm{\ddot{U}}_i = \bm{Z}_i \bm{\ddot{U}}_i = \bm{\ddot{U}}_i$. As previously noted, $\bm{L}_i$ is non-singular, and so the conditions of Lemma \ref{lemma} are satisfied for the matrix difference $(\bm{Z}_i - \bm{\ddot{U}}_i \bm{M_{\ddot{U}}}\bm{\ddot{U}}_i')$. Thus
\begin{align}
\bm{B}_i^+ &= (\bm{Z}_i - \bm{\ddot{U}}_i \bm{M_{\ddot{U}}}\bm{\ddot{U}}_i')^+ \nonumber \\ 
&= \bm{Z}_i + \bm{Z}_i \bm{\ddot{U}}_i \bm{L}_i^{-1} \bm{\ddot{U}}_i' \bm{W}_i \bm{Z}_i \nonumber \\
&= \bm{I}_i - \bm{T}_i \bm{M_T}\bm{T}_i' + \bm{\ddot{U}}_i \bm{L}_i^{-1} \bm{\ddot{U}}_i' \nonumber \\ 
&= \tilde{\bm{B}}_i^+ - \bm{T}_i \bm{M_T}\bm{T}_i'.
\end{align}
Need to show that 
$$
\bm{B}_i^{+1/2} = \tilde{\bm{B}}_i^{+1/2} - \bm{T}_i \bm{M_T}\bm{T}_i'
$$
so that $\bm{A}_i = \tilde{\bm{A}}_i - \bm{T}_i \bm{M_T}\bm{T}_i'$.
