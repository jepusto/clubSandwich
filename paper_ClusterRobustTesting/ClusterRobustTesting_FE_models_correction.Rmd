...
Now, we can show that $\tilde{\bm{A}}_i \bm{T}_i = \bm{T}_i$. Denote the rank of $\bm{\ddot{U}}_i$ as $u_i \leq r + s$ and take the thin QR decomposition of $\bm{\ddot{U}}_i$ as $\bm{\ddot{U}}_i = \bm{Q}_i \bm{R}_i$, where $\bm{Q}_i$ is an $n_i \times u_i$ semi-orthonormal matrix and $\bm{R}_i$ is a $u_i \times r + s$ matrix of rank $u_i$, with $\bm{Q}_i'\bm{Q}_i = \bm{I}$. Note that $\bm{Q}_i'\bm{T}_i = \bm{0}$. Now observe that $\tilde{\bm{B}}_i$ can be written as 
$$
...
\end{equation}
It follows that $\tilde{\bm{A}}_i \bm{T}_i = \tilde{\bm{B}}_i^{+1/2} \bm{T}_i = \bm{T}_i$.
