...
\begin{lem}
\label{lemma}
Let $\bm{G}$ be a $g \times g$ matrix with Moore-Penrose inverse $\bm{G}^+$. Let $\bm{J}$ and $\bm{M}$ be $g \times k$ matrices, and let $\bm{K}$ be a $k \times k$ matrix of full rank. If $\bm{G} \bm{G}^+ \bm{J} = \bm{J}$, $\bm{M}' \bm{G}^+ \bm{G} = \bm{M}'$ and $\bm{R}^{-1} - \bm{M}' \bm{K} \bm{J}$ is non-singular, then
\begin{equation}
\left(\bm{G} - \bm{J} \bm{K} \bm{M}'\right)^+ = \bm{G}^+ + \bm{G}^+ \bm{J}\left( \bm{R}^{-1} - \bm{M}' \bm{K} \bm{J}\right)\bm{M}'\bm{G}^+.
\end{equation}
\end{lem}
...
\begin{equation}
\label{eq:Btilde_i}
\tilde{\bm{B}}_i = \bm{D}_i \left(\bm{I}_i - \bm{\ddot{U}}_i \bm{M_{\ddot{U}}}\bm{\ddot{U}}_i'\bm{W}_i\right) \bs\Psi_i \bm{D}_i'.
\end{equation}
The conditions of Lemma \ref{lemma} hold for the inner term in (\ref{eq:Btilde_i}) because we have assumed that $\bm{L}_i = \bm{M_{\ddot{U}}}^{-1} - \bm{\ddot{U}}_i' \bm{W}_i \bm{\ddot{U}}_i$ is non-singular. Thus, 
\begin{align}
