...
Specifically, for ordinary (i.e., unweighted) least squares estimation with an identity working model $(\bs\Phi = \bm{I})$, then the adjustment matrices can be calculated without accounting for the within-cluster fixed effects, as 
\begin{equation}
\label{eq:CR2_tilde}
...
The main implication of Theorem \ref{thm:absorb} is that the more computationally tractable formula $\bm{\tilde{A}}_i$ can be used in the common case of unweighted least squares estimation.
...
# Appendix C: Further details of the simulations