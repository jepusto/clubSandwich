...
If the working model captures the true form of the error variances, then the weights that make the sampling variance of $\bs{\hat\beta}$ as small as possible (the most precise) are the inverse variance weights $\bs\Psi_j^{-1}$, which depend on the parameter $\bs\theta$. It is recommended that the weights are (approximately) inverse variance to ensure high precision for the sampling variance of $\bs{\hat\beta}$ [@tipton2015meta]. In practice, the analyst will use the inverse of the _estimated_ working model, so that they/we use
$$
...
The CR1, CR1S, and CR1p estimators are variants that use the following constants with $\bmat{E}^1_j = \bmat{W}_j \bmat{X}_j$.
...
\end{aligned}
$$
It is notable that the CR1 estimator is equivalent to the standard linearization estimator $v_{LIN}$ estimator for WLS estimation using the design matrix $\bmat{M} = \left(\sum_{j=1}^J \bmat{X}_j'\bmat{W}_j \bmat{X}_j\right)^{-1}$ [@mccaffrey2001generalizations] and (unweighted) OLS estimation  by using the design matrix $\bmat{M} = \left(\sum_{j=1}^J \bmat{X}_j'\bmat{X}_j\right)^{-1}$ [@bell2002bias]. Because the CR1 estimator can still be biased with a small number of clusters, a class of bias reduced linearization (BRL) variance estimators have been proposed for weighted least squares [@mccaffrey2001generalizations] as well as for OLS estimation with $\bs{\hat\Psi}_j = \bmat{I}, \bmat{W} = \bmat{I}$ [@bell2002bias]. 

<!-- H: Please add a sentence about CR1S and CR1p if you can. I just called them variants of CR1 and nothing more. -->
...
=======
- CR2 uses $f_2 = 1$ and
>>>>>>> ed1435699fcdce8c62d5e2bece32676e792d6540
$$
\bmat{E}_j = \bmat{A}_j' \bmat{W}_j \bmat{X}_j,
$$
where $\bmat{A}_j$ are adjustment matrices that make $\bmat{V}^{CR2}$ _exactly unbiased_ when the working model is _correct_ ($\bs{\Omega} = k \bs{\Psi}$). 

<!-- H: Check if this is the right expression for the working model being correct. -->

To ensure such property, 
$$
\E\left(\bmat{A}_j\bmat{\hat{u}}_j \bmat{\hat{u}}_j' \bmat{A}_j'\right) = \bmat{A}_j \left(\bs\Psi_j - \bmat{X}_j \bmat{M} \bmat{X}_j'\right) \bmat{A}_j' = \bs\Psi_j.
$$
needs to hold for the adjustment matrix $\bmat{A}_j$. (Note that the first equality follows from the distribution of the residuals.)

<!-- Note: McCaffrey and colleagues explain this condition slightly differently with the "outer product of adjusted residuals". We need to clarify the BRL criterion. -->

Then, one possible solution is to use
$$
\bmat{A}_j = \half{\bs\Psi}_j \halfinv{\bmat{B}}_j \half{\bs\Psi}_j,
$$
where 
$$
\bmat{B}_j = \half{\bs\Psi}_j \left(\bs\Psi_j - \bmat{X}_j \bmat{M} \bmat{X}_j'\right) \half{\bs\Psi}_j.
$$

<!-- BRL,S1 and BRL,A1 -->

From [@mccaffrey2001generalizations],

<!-- U (working mat), W (weight mat) -> Psi, W -->
<!-- Check notation below for consistency -->
<<<<<<< HEAD
=======

$$v_{WL} = l' \bmat{M} \left(\sum_{j=1}^J \bmat{X}_j' \bmat{A}_j \bs{r}_j \bs{r}_j' \bmat{A}_j' \bmat{X}_j \right) \bmat{M} l$$

$$ \bmat{A}_j [(\bmat{I} - \bmat{G})_j \bmat{\Psi} (\bmat{I} - \bmat{G})_j'] \bmat{A}_j = \bmat{\Psi}_j \space (4)$$
$$ \bmat{G} = \bmat{X} \bmat{M} \bmat{X}' \bmat{W} $$

<!-- Q. Should we keep the _j notation or should we convert to multiplying c_j anywhere applicable? -->

The solution to (4) is not unique and the bias of resulting variance estimators can differ according to the choice of $\bmat{A}_j$, unless the working covariance matrix is a correct reflection of the true covariance matrix ($\bs{\Omega} = k \bs{\Psi}$) [@mccaffrey2001generalizations].  


...

Still, the symmetric solutions to $\bmat{A}_j\bmat{\hat{u}}_j \bmat{\hat{u}}_j' \bmat{A}_j' = \bs\Psi_j$ or $\bmat{A}_j \bmat{(I - G})_j \bs\Psi_j \bmat{(I - G)}_j' \bmat{A}_j' = \bs\Psi_j$ is useful because symmetric solutions are found to minimize the squared error between the OLS residuals and the the true errors when the errors are i.i.d. Even when the errors were not independent, the symmetric adjustment matrix also yielded the smallest bias [@mccaffrey2001generalizations].
>>>>>>> ed1435699fcdce8c62d5e2bece32676e792d6540
...
$\bmat{A}_j$ satisfying (4) is not unique and the bias of resulting variance estimators can differ according to the choice of $\bmat{A}_j$, unless the working covariance matrix is a correct reflection of the true covariance matrix ($\bs{\Omega} = k \bs{\Psi}$) [@mccaffrey2001generalizations].  
