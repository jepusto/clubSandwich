...
[^niccodemi-trick]: The computational trick consists of applying the following lemma. For a thin $l \times s$ ($l \geq s$) matrix $\bmat{R}$ with full column rank and $\bmat{R}'\bmat{R} \leq \bmat{I}_s$,  $\bmat{R}'(\bmat{I}_l - \bmat{R} \bmat{R}')^a = (\bmat{I}_s - \bmat{R}' \bmat{R})^a \bmat{R}'$ for any $a$, such that the dimensions of inverse or square root matrices can be reduced from $l \times l$ to $s \times s$. To apply this to the CR2f adjustment matrices, we take to $\bmat{R} = \bmat{W}^{1/2}_j \bmat{X}_j \bmat{M}^{C'}$. 
...
```{r}
lm_vcov <- vcovCR(lm_fit, cluster = ChickWeight$Chick, type = "CR2")
sqrt(diag(lm_vcov))

coef_test(lm_fit, vcov = lm_vcov)
```
