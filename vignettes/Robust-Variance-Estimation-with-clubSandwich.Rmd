...
\end{aligned}
$$
It is notable that the CR1 estimator is equivalent to the standard linearization estimator $v_{LIN}$ estimator as described in [@mccaffrey2001generalizations]. The same variance estimator can be applied to unweighted OLS estimation by using the design matrix $\bmat{M} = \left(\sum_{j=1}^J \bmat{X}_j'\bmat{X}_j\right)^{-1}$ instead of $\bmat{M} = \left(\sum_{j=1}^J \bmat{X}_j'\bmat{W}_j \bmat{X}_j\right)^{-1}$ [@bell2002bias]. Because the CR1 estimator can still be biased when the number of clusters is small, a class of bias reduced linearization (BRL) variance estimators have been proposed for weighted least squares [@mccaffrey2001generalizations] as well as for OLS estimation with $\bs{\hat\Psi}_j = \bmat{I}, \bmat{W} = \bmat{I}$ [@bell2002bias]. 
...
$$
where $\bmat{A}_j$ are adjustment matrices that make $\bmat{V}^{CR2}$ _exactly unbiased_ when the working model is _correct_. To ensure such property, 
$$
