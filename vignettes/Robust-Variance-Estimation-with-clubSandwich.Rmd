...
Although not immediately apparent, $\bmat{V}^{CRJ}$ is equivalent to calculating the cluster-robust variance estimator as given in \@ref(eq:V-CR) using $f_{3J} = \frac{J - 1}{J}$ and 
$$
...
(\#eq:jackknife-Aj)
$$
Sometimes, the leading factor in \@ref(eq:jackknife) is omitted, which yields the CR3 estimator. This is equivalent to taking $f_3 = 1$. 
...
<!-- Should we add that CR3$\lambda$ is equivalent to CR3 when clusters are balanced? -->
