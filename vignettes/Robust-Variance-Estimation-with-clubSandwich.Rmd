...
<!-- H: Would we like to explain what problems could arise by using CR0 for cases with a small number of clusters? Or would that be too much detail for this vignette? -->
...
<!-- H: Is there a difference between CR1 and CR1S then? -->

- CR2 uses $f_2 = 1$ and
$$
\bmat{E}_j = \bmat{A}_j \bmat{W}_j \bmat{X}_j,
$$
where $\bmat{A}_j$ are (symmetric) adjustment matrices that make $\bmat{V}^{CR2}$ _exactly unbiased_ when the working model is _correct_. To ensure such property, 
$$
...
<!-- H: Is there a way to express A_j without condensing the middle part as B_j? I suppose that would be too messy a formula though. -->
...
<!-- H: Highlight differences between CR2 and CR3, if any. -->
