...
$$
\bmat{B}_j = \bmat{D}_j \left(\bs\Psi_j - \bmat{X}_j\bmat{M}\bmat{X}_j'\right)\bmat{D}_j'.
$$
...
$$
\bmat{B}_j = \bmat{D}_j \bmat{W}_j \bmat{C}_j\left(\bmat{I}_N - \bmat{H}\right) \bs\Psi \left(\bmat{I}_N - \bmat{H}'\right)\bmat{C}_j \bmat{W}_j \bmat{D}_j'.
$$
...
$$
where $\bmat{A}_j^\sb{3} = \left(\bmat{I} - \bmat{X}_j \bmat{M} \bmat{X}_j' \bmat{W}_j\right)^{-1}$. Sometimes, the leading factor in \@ref(eq:jackknife) is omitted, which yields the CR3 estimator. This is equivalent to taking $f_3 = 1$ and $\bmat{E}_j^\sb{3} = \bmat{E}_j^\sb{J}$.
