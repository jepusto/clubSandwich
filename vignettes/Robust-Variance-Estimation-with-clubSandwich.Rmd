...
The computational trick described by @niccodemi2020refining for the CR2 adjustment under ordinary least squares with an independence working model can be extended to the CR2f adjustment matrices as long as the weight matrices are inverse variance.[^niccodemi-trick] 
...
[^niccodemi-trick]: The computational trick consists of applying the following theorem/lemma/proposition/corollary to $\bmat{\tilde{X}}_j = \bmat{W}^{1/2}_j \bmat{X}_j \bmat{M}^{C'}$ instead of $\bmat{X}_j$. For a thin $l \times s$ ($l \geq s$) matrix $\bmat{R}$ with full column rank and $\bmat{R}'\bmat{R} \leq \bmat{I}_s$,  $\bmat{R}'(\bmat{I}_l - \bmat{R} \bmat{R}')^a = (\bmat{I}_s - \bmat{R}' \bmat{R})^a \bmat{R}'$ for any $a$, such that the dimensions of inverse or square root matrices can be reduced from $l \times l$ to $s \times s$. 

<!-- H: Confirm this footnote together. -->
...
$$
\bmat{E}_j^\sb{3} = \bmat{W}_j \bmat{X}_j \left(\bmat{I}_p - \bmat{M} \bmat{X}_j' \bmat{W}_j \bmat{X}_j \right)^{-1}.
(\#eq:ECR3-quick)
$$
...
(\#eq:CR4-inv)
$$
<!-- Can this formula be simplified further? Apparently not. -->
...
<!-- H: Consult the cran document and lm.R, S3-methods.R under the folder R. -->

- Call and show the example data set (tidyr maybe)

data("ChickWeight", package = "datasets")  # call data set "Chickweight"

- Explain the example model (no intercept, no main effects for Diet)

lm_fit <- lm(weight ~ Time + Diet:Time, data = ChickWeight)

<!-- lm_fit <- lm(weight ~ Time + Diet + Diet:Time, data = ChickWeight) -->
<!-- lm_fit <- lm(weight ~ 0 + Diet + Time:Diet, data = ChickWeight) -->

- Print out the results and explain these are OLS standard errors, not yet cluster-robust.

summary(lm_fit)

- Use vcovCR() in its simplest form and explain the defaults.

vcovCR(lm_fit, cluster = ChickWeight$Chick, type = "CR2")

After fitting a linear model with lm() and storing it into an R object (lm_fit in the example), we can use vcovCR() from the clubSandwich package to estimate a cluster-robust variance-covariance matrix for all regression coefficient estimates in the lm object.
To do so, we need to specify the object and cluster inside vcovCR(). (These are strictly required.)
The object should be the fitted linear regression model, in this case lm_fit.
The cluster should indicate information on which observations among the data set used to fit the regression model (i.e., "ChickWeight" in this example) belong to the same cluster. In this case, we take the variable vector Chick from the data frame ChickWeight.

(The default for vcovCR() when applied to an lm object is the following, which tells ...
...
- Use vcovCR() with more specifications and explain.

vcovCR(lm_fit, cluster = ChickWeight$Chick, type = "CR3")
