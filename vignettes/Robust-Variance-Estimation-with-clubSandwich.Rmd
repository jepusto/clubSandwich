...
\end{aligned}
$$
It is notable that the CR1 estimator is equivalent to the standard linearization estimator $v_{LIN}$ estimator as described in [@mccaffrey2001generalizations]. The same variance estimator can be applied to unweighted OLS estimation by using the design matrix $\bmat{M} = \left(\sum_{j=1}^J \bmat{X}_j'\bmat{X}_j\right)^{-1}$ instead of $\bmat{M} = \left(\sum_{j=1}^J \bmat{X}_j'\bmat{W}_j \bmat{X}_j\right)^{-1}$ [@bell2002bias]. The CR1 estimator can still be biased when the number of clusters is small [@mccaffrey2001generalizations]. Therefore, a class of bias reduced linearization (BRL) variance estimators have been proposed for weighted least squares [@mccaffrey2001generalizations] as well as for OLS estimation ($\bs{\hat\Psi}_j = \bmat{I}, \bmat{W} = \bmat{I}$) [@bell2002bias]. 

<!-- Q: Check if the CR1 estimator is truly the same as the standard linearization estimator (Skinner, 1989; Kott, 1994; v_LIN) mentioned in McCaffrey, Bell, & Botts (2001).
Q: Check citation formats (bibs and how they are referenced) too. I couldn't find the bib for McCaffrey & Bell (2002) so took Bell & McCaffrey (2002) instead.
Q: Confirm that the CR2 is equivalent to the basic BRL estimator. -->

<!-- H: James, please add a sentence about CR1S and CR1p if you can. I just called them variants of CR1 and nothing more. -->
...
<!-- Note: McCaffrey and colleagues explain this condition slightly differently with the "outer product of adjusted residuals". -->
...
<!-- H: Add explanations to other solutions if you see any in the literature. Remember to see if they result in symmetric adjustment matrices. If not, check if the above E_j notation (AWX instead of A'WX) is correct. -->

Still, the symmetric solutions to $\bmat{A}_j\bmat{\hat{u}}_j \bmat{\hat{u}}_j' \bmat{A}_j' = \bs\Psi_j$ or $\bmat{A}_j \bmat{(I - G})_j \bs\Psi_j \bmat{(I - G)}_j' \bmat{A}_j' = \bs\Psi_j$ is useful because ... symmetric solutions are found to minimize the squared error between the OLS residuals and the the true errors when the errors are i.i.d. When the errors were not independent, the symmetric adjustment matrix also yielded the smallest bias (Bell & McCaffrey, 2000/2002).
...
<!-- H: Is this the right way to place the bibs to refer to? I placed them in the bib file (bibliography.bib) as well. -->
