...
# User-defined
zcor <- genZcor(clusz = table(simdatPerm$idvar), waves = wav, corstrv = "unstructured") # nrow = 40, have attributes at the end
geeglm_user <- geeglm(yvar ~ tvar, id = idvar, data = simdatPerm, zcor = zcor, corstr = "userdefined")


test_that("bread works", {
  
  expect_true(check_bread(geeglm_AR1_wav, cluster = simdatPerm$idvar, y = simdatPerm$yvar))
  expect_true(check_bread(geeglm_AR1, cluster = simdat$idvar, y = simdat$yvar))
  ...
  expect_true(check_bread(geeglm_unstr, cluster = simdat$idvar, y = simdat$yvar)) # check bread sandwich
